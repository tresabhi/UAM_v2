...
def vpg(env_fn, 
        actor_critic=core.MLPActorCritic, # Actor-Critic policy
        ac_kwargs=dict(),  
        seed=0, 
        steps_per_epoch=4000, # dataset consisting of data [obs, action, RTG, advantage, lop_probability]
        epochs=50,            # total number of datasets to be collected  
                              #!this should also be the number of times policy network is updated
        gamma=0.99,           # discount factor 
        pi_lr=3e-4,           # policy learning rate
        vf_lr=1e-3,           # value function learning rate
        train_v_iters=80,     #! why is the v_net updated more frequently compared to pi_net  
        lam=0.97,             #
        max_ep_len=1000,      #
        logger_kwargs=dict(), #
        save_freq=10):        #
    ...
    Args:
        ...
        actor_critic: The constructor method for a PyTorch Module with a 
            ...
            module. The ``step`` method should accept a batch of observations #! THE ONLY way this is happening if there are multiple instance of envs 
            and return:
...
if __name__ == '__main__':
    ...
    vpg(lambda : gymnasium.make(args.env), actor_critic=core.MLPActorCritic,
