...
class RVO2_simulator:
    def __init__(self, timestep,radius,max_speed, mapped_env_orca_agent_list:List[UAV_v2_template] = None):
        ...
        self.orca_agent_key_to_mapped_agent:Dict[str, UAV_v2_template] = {}
    ...
    def step(self):
        '''This method needs to talk with MappedEnv, 
           So this method needs to RETURN position info about UAV to MappedEnv'''
        ...
        for orca_agent_key, orca_agent_value in self.orca_agent_dict.items():
            mapped_agent = self.orca_agent_key_to_mapped_agent[orca_agent_key]
            orca_agent_current_position = rvo2.getAgentPosition(self.orca_agent_dict[orca_agent_key])
            self.orca_agent_key_to_mapped_agent[orca_agent_key].current_position = orca_agent_current_position
    ...
    def reset(self,):
        ...
        # agents should come with start and end position attributes
        for mapped_env_orca_agent in self.mapped_env_orca_agent_list:
            ...
            key_str = str(mapped_env_orca_agent.id)
            # mapping agent key str to mapped_agent
            self.orca_agent_key_to_mapped_agent[key_str] = mapped_env_orca_agent
            # adding agent to PyORCA/RVO2 sim
            self.orca_agent_dict[key_str] = self.rvo2_sim.addAgent((x,y))
