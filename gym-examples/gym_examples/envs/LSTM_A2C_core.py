...
class FC(nn.Module):
    def __init__(self, combined_state_input_size, output_size):
        ...
        self.FC_layer = nn.Sequential(
            nn.Linear(combined_state_input_size, output_size),
            nn.ReLU(),
...
class DiscreteActor(Actor):
    def __init__(self, obs_size, discrete_action_size):
        super().__init__()
        self.logits_net = nn.Linear(
            obs_size, discrete_action_size
        )  # check rl_collision implementation - NetworkVPCore, no activation
...
class GausiannActor(Actor):
    def __init__(self, obs_size, continuous_action_size):
        ...
        self.mu_net = nn.Sequential(
            nn.Linear(obs_size, 256), nn.ReLU(), nn.Linear(256, continuous_action_size)
        )
...
class Critic(nn.Module):
    ...
    def forward(self, obs):
        return torch.squeeze(
            self.critic(obs), -1
        )  # TODO: check its output to ensure it returns a scalar/vector with 1 element
...
class ActorCritic(nn.Module):
    ...
    def __init__(self, observation_space, action_space):
        ...
        obs_dim = observation_space.shape[
            0
        ]  # TODO: why shape[0], current implementation is a copy from spinup, check with UAM req

        if isinstance(action_space, Box):
            self.pi = GausiannActor(
                obs_size=obs_dim, continuous_action_size=action_space.shape[0]
            )
        elif isinstance(action_space, Discrete):
            self.pi = DiscreteActor(
                obs_size=obs_dim, discrete_action_size=action_space.shape[0]
            )
...
class LSTM_A2C(nn.Module):
    def __init__(
        self,
        other_agent_states_size,
        learning_agent_state_size,
        lstm_hidden_size,
        FC_output_size,
        action_size,
    ):
        ...
        self.fc_network = FC(
            combined_state_input_size=lstm_hidden_size + learning_agent_state_size,
            output_size=FC_output_size,
        )
        self.a2c = ContinuousActorCritic(
            obs_size=FC_output_size, action_size=action_size
        )
...
if __name__ == "__main__":
    ...
    # Learning model
    lstm_a2c = LSTM_A2C(
        other_agent_states_size=other_agent_state_size,
